...
public interface ProductoDao extends JpaRepository<Producto, Long>{
     ...
     */
     ...
     /**1.Método que devuelve lista de productos ordenados */
     @Query(value = "select p from Producto p left join fetch p.presentacion")
     public List<Producto> findAll(Sort sort);

     /**2. Método que devuelve los productos paginados, una página de producto */

     @Query(value = "select p from Producto p left join fetch p.presentacion", 
     countQuery = "select count(p) from Producto p left join p.presentacion")
    public Page<Producto> findAll(Pageable pageable);

    /**3. Método que recupera un producto por el ID */
